...
@Component({ templateUrl: './edit.user.location.history.component.html', styleUrls: ['./edit.user.location.history.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditUserLocationHistoryComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new UserLocationHistoryEntity();
    ...
    private async loadItem() {
        this.item = new UserLocationHistoryEntity();
        if (this.id) await this.service.item('userlocationhistory', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(UserLocationHistoryEntity, r.Object as UserLocationHistoryEntity);
    ...
    private edit(item: UserLocationHistoryEntity) { this.router.navigate(['/admin/userlocationhistory/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/userlocationhistory' }) } }); }
}
