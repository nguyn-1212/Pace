...
@Component({ templateUrl: './edit.chat.message.component.html', styleUrls: ['./edit.chat.message.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditChatMessageComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new ChatMessageEntity();
    ...
    private async loadItem() {
        this.item = new ChatMessageEntity();
        if (this.id) await this.service.item('chatmessage', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(ChatMessageEntity, r.Object as ChatMessageEntity);
    ...
    private edit(item: ChatMessageEntity) { this.router.navigate(['/admin/chatmessage/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/chatmessage' }) } }); }
}
