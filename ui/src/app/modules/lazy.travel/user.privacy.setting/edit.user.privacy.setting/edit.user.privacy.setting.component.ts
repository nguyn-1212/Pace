...
@Component({ templateUrl: './edit.user.privacy.setting.component.html', styleUrls: ['./edit.user.privacy.setting.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditUserPrivacySettingComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new UserPrivacySettingEntity();
    ...
    private async loadItem() {
        this.item = new UserPrivacySettingEntity();
        if (this.id) await this.service.item('userprivacysetting', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(UserPrivacySettingEntity, r.Object as UserPrivacySettingEntity);
    ...
    private edit(item: UserPrivacySettingEntity) { this.router.navigate(['/admin/userprivacysetting/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/userprivacysetting' }) } }); }
}
