...
@Component({ templateUrl: './edit.user.app.setting.component.html', styleUrls: ['./edit.user.app.setting.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditUserAppSettingComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new UserAppSettingEntity();
    ...
    private async loadItem() {
        this.item = new UserAppSettingEntity();
        if (this.id) await this.service.item('userappsetting', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(UserAppSettingEntity, r.Object as UserAppSettingEntity);
    ...
    private edit(item: UserAppSettingEntity) { this.router.navigate(['/admin/userappsetting/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/userappsetting' }) } }); }
}
