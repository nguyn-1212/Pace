...
@Component({
    templateUrl: './edit.trip.invite.link.component.html',
    styleUrls: ['./edit.trip.invite.link.component.scss', '../../../../../assets/css/modal.scss'],
})
export class EditTripInviteLinkComponent extends EditComponent implements OnInit {
    id: number;
    loading = true;
    item = new TripInviteLinkEntity();
    ...
    private async loadItem() {
        this.item = new TripInviteLinkEntity();
        if (this.id) {
            await this.service.item('tripinvitelink', this.id).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result))
                    this.item = EntityHelper.createEntity(TripInviteLinkEntity, result.Object as TripInviteLinkEntity);
