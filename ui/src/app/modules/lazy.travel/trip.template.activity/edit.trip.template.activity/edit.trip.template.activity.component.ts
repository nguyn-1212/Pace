...
@Component({ templateUrl: './edit.trip.template.activity.component.html', styleUrls: ['./edit.trip.template.activity.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditTripTemplateActivityComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new TripTemplateActivityEntity();
    ...
    private async loadItem() {
        this.item = new TripTemplateActivityEntity();
        if (this.id) await this.service.item('triptemplateactivity', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(TripTemplateActivityEntity, r.Object as TripTemplateActivityEntity);
    ...
    private edit(item: TripTemplateActivityEntity) { this.router.navigate(['/admin/triptemplateactivity/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/triptemplateactivity' }) } }); }
}
