...
@Component({ templateUrl: './edit.trip.doc.component.html', styleUrls: ['./edit.trip.doc.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditTripDocComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new TripDocEntity();
    ...
    private async loadItem() {
        this.item = new TripDocEntity();
        if (this.id) await this.service.item('tripdoc', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(TripDocEntity, r.Object as TripDocEntity);
    ...
    private edit(item: TripDocEntity) { this.router.navigate(['/admin/tripdoc/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/tripdoc' }) } }); }
}
