...
@Component({
    ...
    styleUrls: ['./edit.expense.component.scss', '../../../../../assets/css/modal.scss'],
})
export class EditExpenseComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new ExpenseEntity();
    constructor() {
        super();
    ...
    private async loadItem() {
        this.item = new ExpenseEntity();
        if (this.id) await this.service.item('expense', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(ExpenseEntity, r.Object as ExpenseEntity);
    ...
    private edit(item: ExpenseEntity) {
        this.router.navigate(['/admin/expense/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/expense' }) } });
    }
}
