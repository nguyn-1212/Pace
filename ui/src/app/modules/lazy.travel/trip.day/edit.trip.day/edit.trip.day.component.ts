...
@Component({
    templateUrl: './edit.trip.day.component.html',
    styleUrls: ['./edit.trip.day.component.scss', '../../../../../assets/css/modal.scss'],
})
export class EditTripDayComponent extends EditComponent implements OnInit {
    id: number;
    loading = true;
    item = new TripDayEntity();
    ...
    private async loadItem() {
        this.item = new TripDayEntity();
        if (this.id) {
            await this.service.item('tripday', this.id).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result))
                    this.item = EntityHelper.createEntity(TripDayEntity, result.Object as TripDayEntity);
    ...
    private edit(item: TripDayEntity) {
        const obj = { id: item.Id, prevUrl: '/admin/tripday', prevData: this.state?.prevData };
