...
@Component({
    ...
    styleUrls: ['./edit.user.profile.component.scss', '../../../../../assets/css/modal.scss'],
})
export class EditUserProfileComponent extends EditComponent implements OnInit {
    id: number;
    loading = true;
    item = new UserProfileEntity();
    ...
    private async loadItem() {
        this.item = new UserProfileEntity();
        if (this.id) {
            await this.service.item('userprofile', this.id).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result))
                    this.item = EntityHelper.createEntity(UserProfileEntity, result.Object as UserProfileEntity);
