...
@Component({ templateUrl: './edit.notification.setting.component.html', styleUrls: ['./edit.notification.setting.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditNotificationSettingComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new NotificationSettingEntity();
    ...
    private async loadItem() {
        this.item = new NotificationSettingEntity();
        if (this.id) await this.service.item('notificationsetting', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(NotificationSettingEntity, r.Object as NotificationSettingEntity);
    ...
    private edit(item: NotificationSettingEntity) { this.router.navigate(['/admin/notificationsetting/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/notificationsetting' }) } }); }
}
