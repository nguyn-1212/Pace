...
@Component({
    templateUrl: './edit.expense.split.component.html',
    styleUrls: ['./edit.expense.split.component.scss', '../../../../../assets/css/modal.scss'],
})
export class EditExpenseSplitComponent extends EditComponent implements OnInit {
    id: number;
    loading = true;
    item = new ExpenseSplitEntity();
    ...
    private async loadItem() {
        this.item = new ExpenseSplitEntity();
        if (this.id) {
            await this.service.item('expensesplit', this.id).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result))
                    this.item = EntityHelper.createEntity(ExpenseSplitEntity, result.Object as ExpenseSplitEntity);
