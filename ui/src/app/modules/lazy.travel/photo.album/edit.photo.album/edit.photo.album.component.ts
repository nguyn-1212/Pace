...
@Component({ templateUrl: './edit.photo.album.component.html', styleUrls: ['./edit.photo.album.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditPhotoAlbumComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new PhotoAlbumEntity();
    ...
    private async loadItem() {
        this.item = new PhotoAlbumEntity();
        if (this.id) await this.service.item('photoalbum', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(PhotoAlbumEntity, r.Object as PhotoAlbumEntity);
    ...
    private edit(item: PhotoAlbumEntity) { this.router.navigate(['/admin/photoalbum/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/photoalbum' }) } }); }
}
