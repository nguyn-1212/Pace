...
@Component({ templateUrl: './edit.timeline.entry.component.html', styleUrls: ['./edit.timeline.entry.component.scss', '../../../../../assets/css/modal.scss'] })
export class EditTimelineEntryComponent extends EditComponent implements OnInit {
    id: number; loading = true; item = new TimelineEntryEntity();
    ...
    private async loadItem() {
        this.item = new TimelineEntryEntity();
        if (this.id) await this.service.item('timelineentry', this.id).then((r: ResultApi) => {
            if (ResultApi.IsSuccess(r)) this.item = EntityHelper.createEntity(TimelineEntryEntity, r.Object as TimelineEntryEntity);
    ...
    private edit(item: TimelineEntryEntity) { this.router.navigate(['/admin/timelineentry/edit'], { state: { params: JSON.stringify({ id: item.Id, prevUrl: '/admin/timelineentry' }) } }); }
}
